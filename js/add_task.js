  ...
  if (users[loaduser].contacts && users[loaduser].contacts.length > 0) {
    ...
    for (let i = 1; i < users[loaduser].contacts.length; i++) {
      ...
      if (!searchterm || fullname.toLowerCase().includes(searchterm)) {
        dropdown.innerHTML += generateContactHtml(
          abbreviation,
          fullname,
          color,
          id
...
function selectContacts(entry, id, selected) {
  ...
  entry.children[1].attributes[0].value ===
  "./assets/img/check_button_unchecked.svg"
    ? (entry.children[1].src = "./assets/img/check_button_checked.svg")
    : entry.children[1].attributes[0].value ===
      "./assets/img/check_button_checked.svg"
    ? (entry.children[1].src = "./assets/img/check_button_unchecked.svg")
    : "";
  if (selected === true) {
    ...
    entry.setAttribute(
      "onmousedown",
      `dontChangeFocus(event); selectContacts(this, ${id}, ${!selected})`
    );
  } else {
    selectedcontacts.push(
      users[loaduser].contacts.find((element) => element.id === id)
    );
    entry.setAttribute(
      "onmousedown",
      `dontChangeFocus(event); selectContacts(this, ${id}, ${!selected})`
...
async function addToTasks() {
  ...
  clearAddTaskForm();
  document
    .getElementById("task-added-notification")
    .classList.add("notification-display");
  await delay(1000);
...
async function saveTask() {
  if (sessionStorage.getItem("Guest") === null) {
    ...
    currentTask.prio === "urgent"
      ? users[loaduser].Urgent.push(currentTask)
      : "";
